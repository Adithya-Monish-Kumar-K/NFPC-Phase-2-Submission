...
def train_models(features, cleaned_labels, sd, edges_df, ip_acct_map, branch_acct_map, community_map, city_acct_map=None, state_acct_map=None):
    ...
    # ─── LightGBM (Optuna) ───
    log.info("  LightGBM Optuna tuning (80 trials)...")
    ...
    top_k = min(250, len(all_feat_names))
    ...
    # ─── Stacking (5-model, LGB meta-learner with OOF) ───
    log.info("  Stacking ensemble (5 models, LGB meta)...")
    oof_stack = np.column_stack([lgb_oof, xgb_oof, cb_oof, et_oof, hgb_oof])
    test_stack = np.column_stack([lgb_test, xgb_test, cb_test, et_test, hgb_test])
    meta_params = {
        "objective": "binary", "metric": "auc", "verbosity": -1,
    ...
    oof_models = [lgb_oof, xgb_oof, cb_oof, et_oof, hgb_oof]
    test_models = [lgb_test, xgb_test, cb_test, et_test, hgb_test]
    ...
    best_result = None
    starts = [
        [0.4, 0.25, 0.15, 0.15, 0.05],  # balanced
        [0.5, 0.3, 0.1, 0.1, 0.0],       # LGB+XGB heavy
        [0.3, 0.2, 0.15, 0.25, 0.1],     # ET heavy
        [0.35, 0.25, 0.2, 0.15, 0.05],   # CB emphasis
        [0.25, 0.25, 0.15, 0.25, 0.1],   # equal-ish
    ]
    for s in starts:
    ...
    log.info(f"  Weighted avg AUC: {wavg_auc:.5f} (weights: LGB={best_wavg_w[0]:.4f}, XGB={best_wavg_w[1]:.4f}, CB={best_wavg_w[2]:.4f}, ET={best_wavg_w[3]:.4f}, HGB={best_wavg_w[4]:.4f})")

    all_aucs = {"lgb": lgb_auc, "xgb": xgb_auc, "cb": cb_auc,
                "et": et_auc, "hgb": hgb_auc,
...
def main():
    ...
    # City/State-account mappings for per-fold geographic mule rates
    branch_geo = sd["branch"][["branch_code", "branch_city", "branch_state"]].copy()
    city_acct_map = sd["accounts"][["account_id", "branch_code"]].merge(branch_geo, on="branch_code", how="left")[["account_id", "branch_city"]]
    state_acct_map = sd["accounts"][["account_id", "branch_code"]].merge(branch_geo, on="branch_code", how="left")[["account_id", "branch_state"]]
