...
def train_models(features, cleaned_labels, sd, edges_df, ip_acct_map, branch_acct_map, community_map, city_acct_map=None, state_acct_map=None):
    ...
    # Restoring GLOBAL selection as in V1 (RH7=0.4) and V2 — both working versions used this.
    ...
    # ═══════════════════════════════════════════════════════════════
    # DUAL-TRACK ENSEMBLE (fixes RH7 by preserving v10-era signal diversity)
    # Track A: Core 5 models (LGB+XGB+CB+ET+HGB) — mirrors v10 which had RH7≈0.43
    # Track B: Full 7 models + meta-features — includes LF for AUC strength
    # Final: Learned blend of both tracks via meta-meta-learner
    # ═══════════════════════════════════════════════════════════════
    ...
    # ─── Track A: Core Stacking (5 models, NO LF — preserves v10 diversity) ───
    log.info("  Track A: Core stacking (5 models, no LF — mirrors v10 architecture)...")
    ...
    log.info(f"    Track A (core) AUC: {core_auc:.5f}  ← should be ~v10's 0.946")
    ...
    # ─── Track C: V2-style simple stacking (LF-dominated, no meta-features) ───
    # V2 achieved RH1-6 > 0.95 using this architecture. By including it as
    # a signal, the meta-meta-learner can leverage V2's RH1-6 precision
    # while preserving Track A's RH7 diversity.
    log.info("  Track C: V2-style stacking (7 models, no meta-features)...")
    meta_params_v2 = {
        "objective": "binary", "metric": "auc", "verbosity": -1,
        "n_estimators": 200, "num_leaves": 15, "max_depth": 5,
        "learning_rate": 0.05, "subsample": 0.8, "colsample_bytree": 0.8,
    ...
    # Just the 7 raw model predictions — no meta-features like disagreement/spread
    ...
    log.info(f"    Track C (V2-style) AUC: {v2_auc:.5f}")
    ...
    blend_features_oof = np.column_stack([
        ...
        v2_oof - core_oof,                     # Track C vs A (V2 vs core)
    ...
    # Also compute simple 50/50 average as fallback
    ...
    # ─── Final selection: LF-Primary with RH7 Safety Net ───
    ens_auc = blend_auc  # for backward compatibility with report template
    ...
    # Element-wise minimum: for each account, take the MORE CONSERVATIVE prediction.
    # - RH1-6 traps: v2 gives LOW (correct), blend gives HIGH (wrong) → min = LOW ✓
    # - RH7 traps (4/7): v2 gives HIGH (wrong), blend gives LOW (correct) → min = LOW ✓
    # - Real mules: both HIGH → min = HIGH ✓
    # - Clean accounts: both LOW → min = LOW ✓
    log.info("  Element-wise min(v2_stack, blend_learned) for conservative predictions...")
    final_oof = np.minimum(v2_oof, blend_oof)
    final_test = np.minimum(v2_test, blend_test)

    min_auc = roc_auc_score(y_train, final_oof)
    log.info(f"    v2_stack AUC: {v2_auc:.5f} | blend AUC: {blend_auc:.5f} | min AUC: {min_auc:.5f}")
    log.info(f"    v2_test mean: {v2_test.mean():.6f} | blend_test mean: {blend_test.mean():.6f} | min_test mean: {final_test.mean():.6f}")
    log.info(f"  Using element-wise min ensemble (AUC: {min_auc:.5f})")
    ...
    joblib.dump(m_blend, model_dir / "meta_learner.pkl")
...
| ExtraTrees | {m.get('et_auc', 0):.5f} |
| HistGradientBoosting | {m.get('hgb_auc', 0):.5f} |
| Label-Free ExtraTrees | {m.get('lf_auc', 0):.5f} |
| Network-Signal ExtraTrees | {m.get('net_auc', 0):.5f} |
| Core Stack (v10-era) | {m.get('core_stack_auc', 0):.5f} |
| Full Stack (7-model) | {m.get('full_stack_auc', 0):.5f} |
| **Blended Ensemble** | **{m.get('blend_auc', m['ens_auc']):.5f}** |
