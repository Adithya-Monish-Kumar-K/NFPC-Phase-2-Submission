...
def compute_label_features_for_fold(fold_mule_ids, edges_df, ip_acct_map, branch_acct_map, community_map, all_account_ids, city_acct_map=None, state_acct_map=None):
    ...
    # --- Rank-transform label features (Phase 2: reduce dominance of top features) ---
    for col in ["two_hop_mule_exposure", "shared_ip_ratio", "branch_mule_rate"]:
...
def train_models(features, cleaned_labels, sd, edges_df, ip_acct_map, branch_acct_map, community_map, city_acct_map=None, state_acct_map=None):
    ...
    # ─── Phase 3: Exposure normalization features (distinguish merchants from mules) ───
    log.info("  Adding exposure normalization features...")
    # These combine label-based exposure with base behavioral features
    base_unique_cp = features.reindex(sorted(set(features.index)))["unique_cp"].fillna(1) if "unique_cp" in features.columns else pd.Series(1, index=sorted(set(features.index)))
    base_fanout_cps = features.reindex(sorted(set(features.index)))["fanout_cps"].fillna(1) if "fanout_cps" in features.columns else pd.Series(1, index=sorted(set(features.index)))

    for fold_idx in fold_label_cache:
        lf = fold_label_cache[fold_idx]
        if "two_hop_mule_exposure" in lf.columns:
            exp = lf["two_hop_mule_exposure"]
            lf["exposure_per_cp"] = (exp / (base_unique_cp.reindex(lf.index).fillna(1) + 1)).astype(np.float32)
            lf["exposure_per_activity"] = (exp / (base_fanout_cps.reindex(lf.index).fillna(1) + 1)).astype(np.float32)
        fold_label_cache[fold_idx] = lf

    if "two_hop_mule_exposure" in full_label_feats.columns:
        exp_full = full_label_feats["two_hop_mule_exposure"]
        full_label_feats["exposure_per_cp"] = (exp_full / (base_unique_cp.reindex(full_label_feats.index).fillna(1) + 1)).astype(np.float32)
        full_label_feats["exposure_per_activity"] = (exp_full / (base_fanout_cps.reindex(full_label_feats.index).fillna(1) + 1)).astype(np.float32)
    ...
    study.optimize(lgb_obj, n_trials=80)
    ...
    lgb_model = None
    log.info(f"  LightGBM multi-seed training ({n_seeds} seeds × 5 folds)...")
    for seed_i, ms in enumerate(MULTI_SEEDS):
        seed_lgb_params = lgb_params.copy()
        seed_lgb_params["seed"] = ms
        seed_lgb_params["bagging_seed"] = ms
        seed_lgb_params["feature_fraction_seed"] = ms
        for fold_idx, (ti, vi) in enumerate(fold_splits):
            Xft, Xfv = get_fold_data(fold_idx, ti, vi)
            dt = lgb.Dataset(Xft, y_train[ti], weight=w_train[ti])
            dv = lgb.Dataset(Xfv, y_train[vi], weight=w_train[vi])
            m = lgb.train(seed_lgb_params, dt, valid_sets=[dv],
                          callbacks=[lgb.early_stopping(50, verbose=False), lgb.log_evaluation(-1)])
            lgb_oof[vi] += m.predict(Xfv) / n_seeds
            lgb_test += m.predict(X_te_aug) / (5 * n_seeds)
            if seed_i == 0:
                lgb_model = m  # keep first seed model for feature importance
    lgb_auc = roc_auc_score(y_train, lgb_oof)
    log.info(f"  LightGBM OOF AUC: {lgb_auc:.5f} (multi-seed avg)")
    ...
    top_k = min(220, len(all_feat_names))
    ...
    # ─── XGBoost (Optuna + multi-seed) ───
    log.info("  XGBoost Optuna tuning (60 trials)...")
    ...
    study2.optimize(xgb_obj, n_trials=60)
    ...
    for seed_i in xgb_seeds:
        for fold_idx, (ti, vi) in enumerate(fold_splits):
            Xft, Xfv = get_fold_data(fold_idx, ti, vi)
            m = xgb.XGBClassifier(
                objective="binary:logistic", eval_metric="auc", tree_method="hist",
                device="cuda", n_estimators=2000, verbosity=0, random_state=seed_i, n_jobs=N_JOBS,
                scale_pos_weight=spw,
            ...
            m.fit(Xft, y_train[ti], sample_weight=w_train[ti],
                  eval_set=[(Xfv, y_train[vi])], verbose=False)
            xgb_oof[vi] += m.predict_proba(Xfv)[:, 1] / n_xgb_seeds
            xgb_test += m.predict_proba(X_te_aug)[:, 1] / (5 * n_xgb_seeds)
    xgb_auc = roc_auc_score(y_train, xgb_oof)
    log.info(f"  XGBoost OOF AUC: {xgb_auc:.5f} (multi-seed: {xgb_seeds})")
    ...
    # ─── CatBoost (Optuna) ───
    log.info("  CatBoost Optuna tuning (30 trials)...")
    ...
    try:
        ...
        study3.optimize(cb_obj, n_trials=30)
    ...
    log.info("  ExtraTrees Optuna tuning (20 trials)...")
    ...
    study4.optimize(et_obj, n_trials=20)
...
Label-Free ExtraTrees (base features only — no label leakage, key for RH7) ───
    ...
    # ─── Stacking (6 models + disagreement features, LGB meta-learner with OOF) ───
    log.info("  Stacking ensemble (6 models + disagreement, LGB meta)...")
    all_model_oofs = np.column_stack([lgb_oof, xgb_oof, cb_oof, et_oof, hgb_oof, lf_oof])
    pred_spread = all_model_oofs.max(axis=1) - all_model_oofs.min(axis=1)
    label_aug_mean = np.column_stack([lgb_oof, xgb_oof, cb_oof, et_oof, hgb_oof]).mean(axis=1)
    label_free_delta = lf_oof - label_aug_mean
    oof_stack = np.column_stack([lgb_oof, xgb_oof, cb_oof, et_oof, hgb_oof, lf_oof, pred_spread, label_free_delta])

    all_model_tests = np.column_stack([lgb_test, xgb_test, cb_test, et_test, hgb_test, lf_test_pred])
    test_pred_spread = all_model_tests.max(axis=1) - all_model_tests.min(axis=1)
    test_label_aug_mean = np.column_stack([lgb_test, xgb_test, cb_test, et_test, hgb_test]).mean(axis=1)
    test_label_free_delta = lf_test_pred - test_label_aug_mean
    test_stack = np.column_stack([lgb_test, xgb_test, cb_test, et_test, hgb_test, lf_test_pred, test_pred_spread, test_label_free_deltaientBoostingClassifier(**p)
            m.fit(Xft, y_train[ti], sample_weight=w_train[ti])
            scores.append(roc_auc_score(y_train[vi], m.predict_proba(Xfv)[:, 1]))
        return np.mean(scores)
    ...
    for fold_idx, (ti, vi) in enumerate(fold_splits):
        ...
        m.fit(Xft, y_train[ti], sample_weight=w_train[ti])
, lf_oof]
    test_models = [lgb_test, xgb_test, cb_test, et_test, hgb_test, lf_test_pred]
    ...
    best_result = None
    starts = [
        [0.35, 0.20, 0.15, 0.12, 0.05, 0.13],  # balanced
        [0.40, 0.25, 0.10, 0.10, 0.0, 0.15],    # LGB+XGB heavy
        [0.25, 0.15, 0.15, 0.20, 0.10, 0.15],   # ET heavy
        [0.30, 0.20, 0.20, 0.12, 0.05, 0.13],   # CB emphasis
        [0.20, 0.20, 0.12, 0.20, 0.08, 0.20],   # label-free emphasis30),
            "min_samples_leaf": trial.suggest_int("msl", 5, 100),
            "max_features": trial.suggest_float("mf", 0.3, 1.0),
            "class_weight": "balanced", "random_state": SEED, "n_jobs": N_JOBS,
        }
    ...
    study6 = optuna.create_study(direction="maximize", sampler=optuna.samplers.TPESampler(seed=SEED))
    study6.optimize(lf_et_obj, n_trials=15)
    log.info(f"  Label-Free ET best CV AUC: {study6.best_value:.5f}")
    ...
    all_aucs = {"lgb": lgb_auc, "xgb": xgb_auc, "cb": cb_auc,
                "et": et_auc, "hgb": hgb_auc, "lf": lf_auc,
    ...
    tests = {"lgb": lgb_test, "xgb": xgb_test, "cb": cb_test,
             "et": et_test, "hgb": hgb_test, "lf": lf_test_pred,
             "ensemble": ens_test, "wavg": wavg_test}
    oofs = {"lgb": lgb_oof, "xgb": xgb_oof, "cb": cb_oof,
            "et": et_oof, "hgb": hgb_oof, "lf": lf_train[ti], sample_weight=w_train[ti])
        lf_oof[vi] = m.predict_proba(X_tr[vi])[:, 1]
        lf_test_pred += m.predict_proba(X_te)[:, 1] / 5
    lf_auc = roc_auc_score(y_train, lf_oof)
    log.info(f"  Label-Free ET OOF AUC: {lf_auc:.5f}")
    ...
    # ─── Stacking (6-model + disagreement features, LGB meta-learner with OOF) ───
    log.info("  Stacking ensemble (6 models + disagreement, LGB meta)...")
    # Disagreement features: spread across models and delta between label-free and label-augmented
    all_model_oofs = np.column_stack([lgb_oof, xgb_oof, cb_oof, et_oof, hgb_oof, lf_oof])
    pred_spread = all_model_oofs.max(axis=1) - all_model_oofs.min(axis=1)
    label_aug_mean = np.column_stack([lgb_oof, xgb_oof, cb_oof, et_oof, hgb_oof]).mean(axis=1)
    label_free_delta = lf_oof - label_aug_mean
    ...
    test_stack = np.column_stack([lgb_test, xgb_test, cb_test, et_test, hgb_test, lf_test_pred, test_pred_spread, test_label_free_delta])
    meta_params = {
        "objective": "binary", "metric": "auc", "verbosity": -1,
    ...
    log.info("  Tuning ensemble weights via scipy optimization...")

    oof_models = [lgb_oof, xgb_oof, cb_oof, et_oof, hgb_oof, lf_oof]
    test_models = [lgb_test, xgb_test, cb_test, et_test, hgb_test, lf_test_pred]
    ...
    starts = [
        ...
        [0.25, 0.15, 0.15, 0.20, 0.10, 0.15],   # ET heavy
        [0.30, 0.20, 0.20, 0.12, 0.05, 0.13],   # CB emphasis
        [0.20, 0.20, 0.12, 0.20, 0.08, 0.20],   # label-free emphasis
    ]
    for s in starts:
    ...
    log.info(f"  Weighted avg AUC: {wavg_auc:.5f} (weights: LGB={best_wavg_w[0]:.4f}, XGB={best_wavg_w[1]:.4f}, CB={best_wavg_w[2]:.4f}, ET={best_wavg_w[3]:.4f}, HGB={best_wavg_w[4]:.4f}, LF={best_wavg_w[5]:.4f})")

    all_aucs = {"lgb": lgb_auc, "xgb": xgb_auc, "cb": cb "lf_auc": lf_auc,_auc,
                "et": et_auc, "hgb": hgb_auc, "lf": lf_auc,
                "ensemble": ens_auc, "wavg": wavg_auc}
    best_name = max(all_aucs, key=all_aucs.get)
    best_auc = all_aucs[best_name]
    tests = {"lgb": lgb_test, "xgb": xgb_test, "cb": cb_test,
             ...
             "ensemble": ens_test, "wavg": wavg_test}
    oofs = {"lgb": lgb_oof, "xgb": xgb_oof, "cb": cb_oof,
            "et": et_oof, "hgb": hgb_oof, "lf": lf_oof,
    ...
    log.info(f"  Models saved to {mo7l_dir}")
...
def estimate_windows(results):
    ...
    for i, path in enumerate(TXN_PARTS):
        if i % 100 == 0:
     30)
            ei = min(len(dates) - 1, peak + 30_PARTS)}")
        df = pd.read_parquet(path, columns=["account_id", "transaction_timestamp", "amount"])
...
def main():
    ...
    # City/State-account mappings for per-fold geographic label features
    branch_geo = sd["branch"][["branch_code", "branch_city", "branch_state"]].copy()
    acc_branch = sd["accounts"][["account_id", "branch_code"]].merge(branch_geo, on="branch_code", how="left")
    city_acct_map = acc_branch[["account_id", "branch_city"]].dropna(subset=["branch_city"]).copy()
    state_acct_map = acc_branch[["account_id", "branch_state"]].dropna(subset=["branch_state"]).copy()
    log.info(f"  City/State maps: {len(city_acct_map)} city, {len(state_acct_map)} state entries")
    del acc_branch, branch_geo
    ...
    del cleaned, ip_acct_map, branch_acct_map, community_map, city_acct_map, state_acct_map, all_feats
    gc.collect()
