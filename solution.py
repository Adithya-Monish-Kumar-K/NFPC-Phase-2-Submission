#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AML Mule Account Detection — Solution
==============================================================
Fully vectorized pipeline for processing 400M+ transactions within 16GB RAM.
...
Pipeline Overview:
  ...
  Phase 10: Submission CSV generation

Architecture:
  - 7 diverse base classifiers trained with 5-fold stratified CV
  - Per-fold label feature recomputation to prevent OOF leakage
  - Tri-track meta-stacking (Core, Full, LF-weighted) with meta-meta-learner
  - Isotonic / Platt calibration for well-calibrated probabilities
  - Automatic ensemble selection based on max AUC-ROC

Usage:
  python solution.py  # Auto-detects: uses saved models if available, otherwise runs full training
"""
...
_fh = logging.FileHandler(str(Path(".") / "pipeline3.log"), mode="w", encoding="utf-8")
...
# Automatically set BASE to the directory where the script is run
BASE = Path(".")
CACHE = BASE / "features"
...
def compute_counterparty_reputation(edges_df, contagion_feats):
    ...
    # ── Additional counterparty reputation features ──
...
def clean_labels(sd, features):
    ...
    tl.loc[tl["account_id"].isin(set(routine["account_id"])), "weight"] *= 0.5
...
def compute_flow_chain_features(sd):
    ...
    # ── Additional flow velocity features ──
...
def train_models(features, cleaned_labels, sd, edges_df, ip_acct_map, branch_acct_map, community_map, city_acct_map=None, state_acct_map=None):
    ...
    # Augment test features with full-train label features
    lf_test = full_label_feats.loc[test_ids].values.astype(np.float32)
    X_te_aug = np.column_stack([X_te, lf_test])
    log.info(f"  Augmented feature dim: {len(all_feat_names)} (base {len(feat_names)} + label {len(label_col_names)})")
    ...
    def get_fold_data(fold_idx, ti, vi):
        ...
        X_fold_train = np.column_stack([X_tr[ti], lf_train])
        X_fold_val = np.column_stack([X_tr[vi], lf_val])
    ...
    # Select top-k features by SHAP/gain importance to reduce noise and improve generalization
    ...
    if n_dropped > 0:
        ...
        # Rebuild pruned data for XGB/CatBoost/ET/HGB
    ...
    # ─── Label-Free ExtraTrees (base features only — no label leakage, key for RH7) ───
    log.info("  Label-Free ExtraTrees (base features only, 10 Optuna trials)...")

    def lf_et_obj(trial):
        p = {
            "n_estimators": trial.suggest_int("ne", 300, 1200, step=100),
            "max_depth": trial.suggest_int("md", 8, 25),
            "min_samples_leaf": trial.suggest_int("msl", 10, 100),
            "max_features": trial.suggest_float("mf", 0.3, 0.8),
            "class_weight": "balanced", "random_state": SEED, "n_jobs": N_JOBS,
        }
        scores = []
        for fold_idx, (ti, vi) in enumerate(fold_splits):
            m = ExtraTreesClassifier(**p)
            m.fit(X_tr[ti], y_train[ti], sample_weight=w_train[ti])
            scores.append(roc_auc_score(y_train[vi], m.predict_proba(X_tr[vi])[:, 1]))
        return np.mean(scores)

    study6 = optuna.create_study(direction="maximize", sampler=optuna.samplers.TPESampler(seed=SEED))
    study6.optimize(lf_et_obj, n_trials=10)
    log.info(f"  Label-Free ET best CV AUC: {study6.best_value:.5f}")

    bp6 = study6.best_params
    lf_oof = np.zeros(len(X_tr))
    lf_test_pred = np.zeros(len(X_te))
    lf_fold_models = []
    for fold_idx, (ti, vi) in enumerate(fold_splits):
        m = ExtraTreesClassifier(
            n_estimators=bp6["ne"], max_depth=bp6["md"],
            min_samples_leaf=bp6["msl"], max_features=bp6["mf"],
            class_weight="balanced", random_state=SEED, n_jobs=N_JOBS,
        )
        m.fit(X_tr[ti], y_train[ti], sample_weight=w_train[ti])
        lf_oof[vi] = m.predict_proba(X_tr[vi])[:, 1]
        lf_test_pred += m.predict_proba(X_te)[:, 1] / 5
        lf_fold_models.append(m)
    lf_auc = roc_auc_score(y_train, lf_oof)
    log.info(f"  Label-Free ET OOF AUC: {lf_auc:.5f}")

    del study6
    gc.collect()

    # ─── Network-Signal ExtraTrees (graph+label features only — detects RH7) ───
    log.info("  Network-Signal ExtraTrees (graph+label features, 10 Optuna trials)...")

    # Identify network/graph/label feature columns in the augmented feature space
    network_feat_patterns = [
        "degree", "weighted_degree", "edge_", "counterparty_", "top_cp",
        "fanin", "fanout", "herfindahl", "pagerank", "betweenness", "clustering_coeff",
        "community_", "contagion_", "incoming_risk", "outgoing_risk",
        "risk_neighbor", "risk_cluster", "shared_cp", "suspicious_cp",
        "two_hop", "shared_ip", "branch_mule", "branch_collusion", "branch_susp",
        "avg_cp_contagion", "max_cp_contagion", "high_risk_cp",
        "std_cp_contagion", "min_cp_contagion", "cp_risk_range",
        "weighted_cp_risk", "incoming_risk_ratio", "city_mule_rate", "state_mule_rate",
    ]
    net_col_idx = np.array([i for i, f in enumerate(all_feat_names)
                            if any(f.startswith(p) or p in f for p in network_feat_patterns)])
    log.info(f"    Network feature columns: {len(net_col_idx)} / {len(all_feat_names)}")

    if len(net_col_idx) >= 5:
        def net_et_obj(trial):
            p = {
                "n_estimators": trial.suggest_int("ne", 300, 1200, step=100),
                "max_depth": trial.suggest_int("md", 6, 20),
                "min_samples_leaf": trial.suggest_int("msl", 10, 100),
                "max_features": trial.suggest_float("mf", 0.3, 1.0),
                "class_weight": "balanced", "random_state": SEED, "n_jobs": N_JOBS,
            }
            ...
            for fold_idx, (ti, vi) in enumerate(fold_splits):
                Xft, Xfv = get_fold_data(fold_idx, ti, vi)
                Xft_net = Xft[:, net_col_idx]
                Xfv_net = Xfv[:, net_col_idx]
                m = ExtraTreesClassifier(**p)
                m.fit(Xft_net, y_train[ti], sample_weight=w_train[ti])
                scores.append(roc_auc_score(y_train[vi], m.predict_proba(Xfv_net)[:, 1]))
            return np.mean(scores)

        study7 = optuna.create_study(direction="maximize", sampler=optuna.samplers.TPESampler(seed=SEED))
        study7.optimize(net_et_obj, n_trials=10)
        log.info(f"  Network-Signal ET best CV AUC: {study7.best_value:.5f}")

        bp7 = study7.best_params
        net_oof = np.zeros(len(X_tr))
        net_test_pred = np.zeros(len(X_te))
        net_fold_models = []
        for fold_idx, (ti, vi) in enumerate(fold_splits):
            Xft, Xfv = get_fold_data(fold_idx, ti, vi)
            Xft_net = Xft[:, net_col_idx]
            Xfv_net = Xfv[:, net_col_idx]
            m = ExtraTreesClassifier(
                n_estimators=bp7["ne"], max_depth=bp7["md"],
                min_samples_leaf=bp7["msl"], max_features=bp7["mf"],
                class_weight="balanced", random_state=SEED, n_jobs=N_JOBS,
            )
            m.fit(Xft_net, y_train[ti], sample_weight=w_train[ti])
            net_oof[vi] = m.predict_proba(Xfv_net)[:, 1]
            net_test_pred += m.predict_proba(X_te_aug[:, net_col_idx])[:, 1] / 5
            net_fold_models.append(m)
        net_auc = roc_auc_score(y_train, net_oof)
        log.info(f"  Network-Signal ET OOF AUC: {net_auc:.5f}")
        del study7
    else:
        log.warning("  Not enough network features found — skipping Network-Signal ET")
        net_oof = np.zeros(len(X_tr))
        net_test_pred = np.zeros(len(X_te))
        net_auc = 0.5
        net_fold_models = []
    ...
    # ═══════════════════════════════════════════════════════════════
    # TRI-TRACK META-STACKING ENSEMBLE
    # Track A: Core 5 models (LGB+XGB+CB+ET+HGB) — captures label-augmented signal diversity
    # Track B: Full 7 models + meta-features — maximizes AUC via LF and network signals
    # Track C: LF-weighted 7-model stacking — emphasizes label-free generalization
    # Final: Learned blend of all three tracks via meta-meta-learner
    # ═══════════════════════════════════════════════════════════════

    meta_params_core = {
        "objective": "binary", "metric": "auc", "verbosity": -1,
        "n_estimators": 100, "num_leaves": 10, "max_depth": 4,
        "learning_rate": 0.05, "subsample": 0.8, "colsample_bytree": 0.8,
        "reg_lambda": 1.0, "scale_pos_weight": spw, "seed": SEED,
    }

    # ─── Track A: Core Stacking (5 label-augmented models for signal diversity) ───
    log.info("  Track A: Core stacking (5 models, label-augmented only)...")
    core_oofs = np.column_stack([lgb_oof, xgb_oof, cb_oof, et_oof, hgb_oof])
    core_spread = core_oofs.max(axis=1) - core_oofs.min(axis=1)
    core_std = core_oofs.std(axis=1)
    oof_core_stack = np.column_stack([lgb_oof, xgb_oof, cb_oof, et_oof, hgb_oof,
                                      core_spread, core_std])

    core_tests = np.column_stack([lgb_test, xgb_test, cb_test, et_test, hgb_test])
    test_core_spread = core_tests.max(axis=1) - core_tests.min(axis=1)
    test_core_std = core_tests.std(axis=1)
    test_core_stack = np.column_stack([lgb_test, xgb_test, cb_test, et_test, hgb_test,
                                       test_core_spread, test_core_std])

    core_oof = np.zeros(len(X_tr))
    core_test = np.zeros(len(X_te))
    core_meta_models = []
    for ti, vi in fold_splits:
        dt = lgb.Dataset(oof_core_stack[ti], y_train[ti], weight=w_train[ti])
        dv = lgb.Dataset(oof_core_stack[vi], y_train[vi], weight=w_train[vi])
        m_core = lgb.train(meta_params_core, dt, valid_sets=[dv],
                           callbacks=[lgb.early_stopping(30, verbose=False), lgb.log_evaluation(-1)])
        core_oof[vi] = m_core.predict(oof_core_stack[vi])
        core_test += m_core.predict(test_core_stack) / 5
        core_meta_models.append(m_core)
    core_auc = roc_auc_score(y_train, core_oof)
    log.info(f"    Track A (core) AUC: {core_auc:.5f}")

    # ─── Track B: Full Stacking (7 models + rich meta-features) ───
    log.info("  Track B: Full stacking (7 models + meta-features)...")
    all_model_oofs = np.column_stack([lgb_oof, xgb_oof, cb_oof, et_oof, hgb_oof, lf_oof, net_oof])
    pred_spread = all_model_oofs.max(axis=1) - all_model_oofs.min(axis=1)
    prediction_std = all_model_oofs.std(axis=1)
    label_aug_mean = core_oofs.mean(axis=1)
    label_free_delta = lf_oof - label_aug_mean
    net_vs_lf_delta = net_oof - lf_oof
    network_confidence = net_oof * (1.0 - lf_oof)
    ...
    oof_full_stack = np.column_stack([
        lgb_oof, xgb_oof, cb_oof, et_oof, hgb_oof, lf_oof, net_oof,
        pred_spread, prediction_std, label_free_delta, net_vs_lf_delta,
        network_confidence,
    ])

    all_model_tests = np.column_stack([lgb_test, xgb_test, cb_test, et_test, hgb_test, lf_test_pred, net_test_pred])
    test_pred_spread = all_model_tests.max(axis=1) - all_model_tests.min(axis=1)
    test_prediction_std = all_model_tests.std(axis=1)
    test_label_aug_mean = core_tests.mean(axis=1)
    test_label_free_delta = lf_test_pred - test_label_aug_mean
    test_net_vs_lf_delta = net_test_pred - lf_test_pred
    test_network_confidence = net_test_pred * (1.0 - lf_test_pred)

    test_full_stack = np.column_stack([
        lgb_test, xgb_test, cb_test, et_test, hgb_test, lf_test_pred, net_test_pred,
        test_pred_spread, test_prediction_std, test_label_free_delta,
        test_net_vs_lf_delta, test_network_confidence,
    ])

    full_oof = np.zeros(len(X_tr))
    full_test = np.zeros(len(X_te))
    full_meta_models = []
    for ti, vi in fold_splits:
        dt = lgb.Dataset(oof_full_stack[ti], y_train[ti], weight=w_train[ti])
        dv = lgb.Dataset(oof_full_stack[vi], y_train[vi], weight=w_train[vi])
        m_full = lgb.train(meta_params_full, dt, valid_sets=[dv],
                           callbacks=[lgb.early_stopping(30, verbose=False), lgb.log_evaluation(-1)])
        full_oof[vi] = m_full.predict(oof_full_stack[vi])
        full_test += m_full.predict(test_full_stack) / 5
        full_meta_models.append(m_full)
    full_auc = roc_auc_score(y_train, full_oof)
    log.info(f"    Track B (full) AUC: {full_auc:.5f}")

    # ─── Track C: LF-weighted stacking (7 models, emphasizes label-free generalization) ───
    # This track stacks all 7 models with an extra LF-weight signal, encouraging
    # the meta-learner to leverage label-free model predictions for generalization
    # on unseen (test) accounts not influenced by label leakage.
    log.info("  Track C: LF-weighted stacking (7 models, label-free emphasis)...")
    meta_params_v2 = {
        "objective": "binary", "metric": "auc", "verbosity": -1,
        "n_estimators": 200, "num_leaves": 15, "max_depth": 5,
        "learning_rate": 0.05, "subsample": 0.8, "colsample_bytree": 0.8,
    ...
    # 7 raw model predictions + LF-weight feature for label-free emphasis
    oof_v2_stack = np.column_stack([lgb_oof, xgb_oof, cb_oof, et_oof, hgb_oof, lf_oof, net_oof])
    test_v2_stack = np.column_stack([lgb_test, xgb_test, cb_test, et_test, hgb_test, lf_test_pred, net_test_pred])

    v2_oof = np.zeros(len(X_tr))
    v2_test = np.zeros(len(X_te))
    v2_meta_models = []
    for ti, vi in fold_splits:
        dt = lgb.Dataset(oof_v2_stack[ti], y_train[ti], weight=w_train[ti])
        dv = lgb.Dataset(oof_v2_stack[vi], y_train[vi], weight=w_train[vi])
        m_v2 = lgb.train(meta_params_v2, dt, valid_sets=[dv],
                         callbacks=[lgb.early_stopping(30, verbose=False), lgb.log_evaluation(-1)])
        v2_oof[vi] = m_v2.predict(oof_v2_stack[vi])
        v2_test += m_v2.predict(test_v2_stack) / 5
        v2_meta_models.append(m_v2)
    v2_auc = roc_auc_score(y_train, v2_oof)
    log.info(f"    Track C (LF-weighted) AUC: {v2_auc:.5f}")

    # ─── Blend: Meta-meta-learner on THREE tracks ───
    log.info("  Blending Track A + Track B + Track C via meta-meta-learner...")
    # Features: all three track outputs + disagreements + raw signals
    core_avg_oof = core_oofs.mean(axis=1)
    core_avg_test = core_tests.mean(axis=1)
    blend_features_oof = np.column_stack([
        core_oof, full_oof, v2_oof,            # all three tracks
        core_oof - full_oof,                   # Track A vs B disagreement
        v2_oof - core_oof,                     # Track C vs A disagreement
        core_avg_oof,                          # raw core average
        lf_oof,                                # LF raw (strongest single model)
        net_oof,                               # Network raw (RH7 detection)
        lf_oof - core_avg_oof,                 # LF vs core disagreement
    ])
    blend_features_test = np.column_stack([
        core_test, full_test, v2_test,
        core_test - full_test,
        v2_test - core_test,
        core_avg_test,
        lf_test_pred,
        net_test_pred,
        lf_test_pred - core_avg_test,
    ])

    blend_params = {
        "objective": "binary", "metric": "auc", "verbosity": -1,
        "n_estimators": 80, "num_leaves": 8, "max_depth": 3,
        "learning_rate": 0.05, "subsample": 0.8, "colsample_bytree": 0.8,
        "reg_lambda": 2.0, "scale_pos_weight": spw, "seed": SEED,
    }
    blend_oof = np.zeros(len(X_tr))
    blend_test = np.zeros(len(X_te))
    blend_meta_models = []
    for ti, vi in fold_splits:
        dt = lgb.Dataset(blend_features_oof[ti], y_train[ti], weight=w_train[ti])
        dv = lgb.Dataset(blend_features_oof[vi], y_train[vi], weight=w_train[vi])
        m_blend = lgb.train(blend_params, dt, valid_sets=[dv],
                            callbacks=[lgb.early_stopping(20, verbose=False), lgb.log_evaluation(-1)])
        blend_oof[vi] = m_blend.predict(blend_features_oof[vi])
        blend_test += m_blend.predict(blend_features_test) / 5
        blend_meta_models.append(m_blend)
    blend_auc = roc_auc_score(y_train, blend_oof)
    log.info(f"    Blended AUC: {blend_auc:.5f}")

    # Weighted average blend as a robust fallback ensemble
    simple_blend_oof = 0.5 * core_oof + 0.5 * full_oof
    simple_blend_test = 0.5 * core_test + 0.5 * full_test
    simple_blend_auc = roc_auc_score(y_train, simple_blend_oof)
    log.info(f"    Simple 50/50 blend AUC: {simple_blend_auc:.5f}")

    # ─── Final selection: best ensemble method, NEVER a single model ───
    ens_auc = blend_auc
    wavg_auc = simple_blend_auc

    all_aucs = {"lgb": lgb_auc, "xgb": xgb_auc, "cb": cb_auc,
                "et": et_auc, "hgb": hgb_auc, "lf": lf_auc, "net": net_auc,
                "core_stack": core_auc, "full_stack": full_auc, "v2_stack": v2_auc,
                "blend_learned": blend_auc, "blend_simple": simple_blend_auc}
    log.info(f"  All AUCs: {' | '.join(f'{k}={v:.5f}' for k, v in all_aucs.items())}")

    # Pick the best ensemble method (never single model)
    ensemble_options = {
        "blend_learned": (blend_oof, blend_test, blend_auc),
        "blend_simple": (simple_blend_oof, simple_blend_test, simple_blend_auc),
        "core_stack": (core_oof, core_test, core_auc),
        "full_stack": (full_oof, full_test, full_auc),
        "v2_stack": (v2_oof, v2_test, v2_auc),
    }
    best_name = max(ensemble_options, key=lambda k: ensemble_options[k][2])
    final_oof, final_test, _ = ensemble_options[best_name]
    log.info(f"  Using {best_name} (AUC: {all_aucs[best_name]:.5f}) — tri-track ensemble")
    ...
    # ─── Save Models ───
    log.info("  Saving ALL fold model artefacts for inference mode...")
    model_dir = BASE / "models"
    model_dir.mkdir(exist_ok=True)
    # Base fold models (7 algorithms × 5 folds)
    joblib.dump(lgb_fold_models, model_dir / "lgb_fold_models.pkl")
    joblib.dump(xgb_fold_models, model_dir / "xgb_fold_models.pkl")
    joblib.dump(cb_fold_models, model_dir / "cb_fold_models.pkl")
    joblib.dump(et_fold_models, model_dir / "et_fold_models.pkl")
    joblib.dump(hgb_fold_models, model_dir / "hgb_fold_models.pkl")
    joblib.dump(lf_fold_models, model_dir / "lf_fold_models.pkl")
    joblib.dump(net_fold_models, model_dir / "net_fold_models.pkl")
    # Meta-learner fold models (3 tracks × 5 folds + blend × 5 folds)
    joblib.dump(core_meta_models, model_dir / "core_meta_models.pkl")
    joblib.dump(v2_meta_models, model_dir / "v2_meta_models.pkl")
    joblib.dump(blend_meta_models, model_dir / "blend_meta_models.pkl")
    # full_stack meta models (Track B)
    joblib.dump(full_meta_models, model_dir / "full_meta_models.pkl")
    # Calibrators + metadata
    joblib.dump(iso, model_dir / "isotonic_calibrator.pkl")
    if cal_method == 'platt':
        joblib.dump(platt, model_dir / "platt_calibrator.pkl")
    joblib.dump(all_feat_names, model_dir / "feature_names.pkl")
    joblib.dump(best_name, model_dir / "best_ensemble_name.pkl")
    joblib.dump(best_thresh, model_dir / "optimal_threshold.pkl")
    joblib.dump(cal_method, model_dir / "cal_method.pkl")
    if n_dropped > 0:
        joblib.dump(sel_idx_arr.tolist(), model_dir / "selected_feature_idx.pkl")
    joblib.dump(net_col_idx.tolist() if len(net_col_idx) > 0 else [], model_dir / "net_col_idx.pkl")
    # Save single LightGBM model for SHAP compatibility
    joblib.dump(lgb_model, model_dir / "lgb_model.pkl")
    total_size = sum(f.stat().st_size for f in model_dir.glob('*.pkl')) / (1024**2)
    log.info(f"  Models saved to {model_dir} ({total_size:.1f} MB total)")
    ...
    return {
        ...
        "metrics": {"lgb_auc": lgb_auc, "xgb_auc": xgb_auc, "cb_auc": cb_auc,
                     "et_auc": et_auc, "hgb_auc": hgb_auc, "lf_auc": lf_auc,
                     "net_auc": net_auc,
                     "core_stack_auc": core_auc, "full_stack_auc": full_auc,
                     "ens_auc": blend_auc, "wavg_auc": simple_blend_auc,
                     "blend_auc": blend_auc,
...
def estimate_windows(results):
    import ruptures as rpt

    log.info("PHASE 5: Temporal windows...")
    ...
    for i, path in enumerate(TXN_PARTS):
        ...
        df = df[df["account_id"].isin(suspicious)]
        if len(df) == 0:
            continue
    ...
    # Pre-group for O(1) per-account lookup instead of O(n) filter
    ...
    for aid in suspicious:
        ...
        if len(ad) == 0:
            ...
            continue
        if len(ad) < 10:
            # Few data points — use full transaction range as window
            windows.append({
                "account_id": aid,
...
def generate_submission(results, windows, sd, risk_scores=None):
    ...
    # Save both the full CSV (with risk_level) and the submission-format CSV (4 columns)
    sub_full = sub[["account_id", "is_mule", "risk_level", "suspicious_start", "suspicious_end"]]
    sub_full.to_csv(BASE / "risk.csv", index=False)
    ...
    for lvl in ["CRITICAL", "HIGH", "MEDIUM", "LOW", "NORMAL"]:
        log.info(f"  {lvl}: {(sub_full['risk_level'] == lvl).sum()}")
    return sub


def build_all_features(sd):
    """Run full feature engineering pipeline, returning (all_feats, edges_df).

    Each compute_* function has its own internal cache, so re-runs are fast
    if intermediate parquets already exist.
    """
    _feat_cache = CACHE / "all_features_v8.parquet"
    edges_cache = CACHE / "edges_cache.parquet"

    if _feat_cache.exists() and edges_cache.exists():
        log.info("Loading cached features + edges...")
        return pd.read_parquet(_feat_cache), pd.read_parquet(edges_cache)

    log.info("Running full feature engineering pipeline...")

    # Phase 1: Transaction features
    txn_feats = compute_txn_features(sd)

    # Phase 2: Static features
    static_feats = compute_static_features(sd)

    # Phase 3: Graph/network features (also saves edges_cache.parquet)
    graph_feats = compute_graph_features(sd)

    # Phase 3b: Geo/IP/balance features (also saves ip_pairs_cache.parquet)
    geo_feats = compute_geo_features(sd)

    # Phase 3c: PTT features
    ptt_feats = compute_ptt_features(sd)

    # Phase 3d: MCC features
    mcc_feats = compute_mcc_features(sd)

    # Phase 3e: Composite features
    composite_feats = compute_composite_features(static_feats, txn_feats, graph_feats, geo_feats)

    # Phase 3f: Graph embeddings (also saves community_map_cache.parquet)
    emb_feats = compute_graph_embeddings(sd)

    # Phase 4: Temporal graph features
    temporal_feats = compute_temporal_graph_features(sd)

    # Phase 4b: Flow chain features
    flow_feats = compute_flow_chain_features(sd)

    # Join all feature sets
    log.info("Joining all feature sets...")
    all_ids = sd["all_ids"]
    all_feats = pd.DataFrame(index=all_ids)
    all_feats.index.name = "account_id"

    for feat_df in [txn_feats, static_feats, graph_feats, geo_feats, ptt_feats,
                    mcc_feats, composite_feats, emb_feats, temporal_feats, flow_feats]:
        all_feats = all_feats.join(feat_df, how="left")

    del txn_feats, static_feats, graph_feats, geo_feats, ptt_feats
    del mcc_feats, composite_feats, emb_feats, temporal_feats, flow_feats
    gc.collect()

    # Phase 5: Anomaly detection (needs joined features)
    anomaly_feats = compute_anomaly_features(all_feats)
    all_feats = all_feats.join(anomaly_feats, how="left")
    del anomaly_feats; gc.collect()

    # Phase 5b: Behavioral screening
    screening_feats = compute_behavioral_screening(all_feats)
    all_feats = all_feats.join(screening_feats, how="left")
    del screening_feats; gc.collect()

    # Phase 5c: Risk contagion
    edges_df = pd.read_parquet(edges_cache)
    contagion_feats = compute_risk_contagion_features(all_feats, edges_df)
    all_feats = all_feats.join(contagion_feats, how="left")

    # Phase 5d: Counterparty reputation
    cp_rep_feats = compute_counterparty_reputation(edges_df, contagion_feats)
    all_feats = all_feats.join(cp_rep_feats, how="left")
    del contagion_feats, cp_rep_feats; gc.collect()

    # Clean up
    all_feats = all_feats.fillna(0).replace([np.inf, -np.inf], 0)

    # Drop non-numeric columns
    obj_cols = all_feats.select_dtypes(include=["object", "datetime64"]).columns.tolist()
    if obj_cols:
        all_feats = all_feats.drop(columns=obj_cols)

    all_feats.to_parquet(_feat_cache)
    log.info(f"  Feature engineering complete: {all_feats.shape}")

    return all_feats, edges_df


def main():
    log.info("=" * 60)
    log.info("AML Intelligence Platform — START")
    log.info("=" * 60)

    sd = load_static()

    # Feature engineering (will load from cache if already built)
    all_feats, edges_df = build_all_features(sd)
    all_feats.index.name = "account_id"
    log.info(f"  Feature matrix: {all_feats.shape}")
    ...
    # Load IP pairs as compact per-account mapping (memory-efficient hashed representation)
    ip_acct_map = None
    ...
    # Branch/city/state-account mappings for per-fold label features
    branch_acct_map = sd["accounts"][["account_id", "branch_code"]].copy()
    city_acct_map = sd["accounts"][["account_id", "city"]].copy() if "city" in sd["accounts"].columns else None
    state_acct_map = sd["accounts"][["account_id", "state"]].copy() if "state" in sd["accounts"].columns else None
    ...
    # Temporal windows
    windows = estimate_windows(results)
    ...
    # Save pipeline metadata for external report generation
    import json
    _meta = {
        "metrics": results["metrics"],
        "n_features": len(results.get("feat_names", [])),
        "feat_names": results.get("feat_names", []),
        "train_ids": [str(x) for x in results["train_ids"]],
        "test_ids": [str(x) for x in results["test_ids"]],
        "test_preds": [float(x) for x in results["test_preds"]],
        "y_train": [int(x) for x in results["y_train"]],
    }
    (CACHE / "pipeline_metadata.json").write_text(json.dumps(_meta), encoding="utf-8")
    log.info("  Pipeline metadata saved to features/pipeline_metadata.json")

    log.info("=" * 60)
    log.info("PIPELINE COMPLETE — AML Intelligence Platform")
    log.info("=" * 60)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["train", "predict"], default=None,
                        help="'train' = full training pipeline; 'predict' = instant inference from saved models. "
                             "If omitted, auto-detects based on presence of models/*.pkl files.")
    args = parser.parse_args()

    model_dir = BASE / "models"

    # Decide mode: explicit flag wins, otherwise auto-detect
    if args.mode == "train":
        run_predict = False
    elif args.mode == "predict":
        run_predict = True
    else:
        run_predict = model_dir.exists() and any(model_dir.glob("*.pkl"))

    if run_predict:
        # ─── Instant Inference from Saved Models ───
        if not model_dir.exists() or not any(model_dir.glob("*.pkl")):
            log.error("PREDICT MODE requires trained models in models/ directory.")
            log.error("Run 'python solution.py --mode train' first to generate them.")
            raise SystemExit(1)

        import joblib
        log.info("=" * 60)
        log.info("PREDICT MODE — Instant Inference from Saved Models")
        log.info("=" * 60)

        # Load models
        log.info("  Loading saved models...")
        lgb_fold_models = joblib.load(model_dir / "lgb_fold_models.pkl")
        xgb_fold_models = joblib.load(model_dir / "xgb_fold_models.pkl")
        cb_fold_models = joblib.load(model_dir / "cb_fold_models.pkl")
        et_fold_models = joblib.load(model_dir / "et_fold_models.pkl")
        hgb_fold_models = joblib.load(model_dir / "hgb_fold_models.pkl")
        lf_fold_models = joblib.load(model_dir / "lf_fold_models.pkl")
        net_fold_models = joblib.load(model_dir / "net_fold_models.pkl")
        core_meta_models = joblib.load(model_dir / "core_meta_models.pkl")
        full_meta_models = joblib.load(model_dir / "full_meta_models.pkl")
        v2_meta_models = joblib.load(model_dir / "v2_meta_models.pkl")
        blend_meta_models = joblib.load(model_dir / "blend_meta_models.pkl")
        iso = joblib.load(model_dir / "isotonic_calibrator.pkl")
        platt = joblib.load(model_dir / "platt_calibrator.pkl") if (model_dir / "platt_calibrator.pkl").exists() else None
        feature_names = joblib.load(model_dir / "feature_names.pkl")
        best_ensemble_name = joblib.load(model_dir / "best_ensemble_name.pkl")
        optimal_threshold = joblib.load(model_dir / "optimal_threshold.pkl")
        cal_method = joblib.load(model_dir / "cal_method.pkl")
        sel_idx = joblib.load(model_dir / "selected_feature_idx.pkl") if (model_dir / "selected_feature_idx.pkl").exists() else None
        net_col_idx = np.array(joblib.load(model_dir / "net_col_idx.pkl"))
        log.info(f"  Models loaded. Best ensemble: {best_ensemble_name}, Threshold: {optimal_threshold:.2f}")

        # Load or generate features
        log.info("  Checking for cached features...")
        sd = load_static()
        _feat_cache = CACHE / "all_features_v8.parquet"
        _edges_cache = CACHE / "edges_cache.parquet"

        if not _feat_cache.exists() or not _edges_cache.exists():
            log.warning("  Feature/edges cache not found. Running full feature engineering from scratch...")
            log.warning("  This will take ~30-45 minutes depending on hardware.")
            all_feats, edges_df = build_all_features(sd)
            all_feats.to_parquet(_feat_cache)
            log.info(f"  Saved generated features to {_feat_cache.name}")
        else:
            log.info(f"  Loading cached features from {_feat_cache.name}...")
            all_feats = pd.read_parquet(_feat_cache)
            edges_df = pd.read_parquet(_edges_cache)

        all_feats.index.name = "account_id"
        test_ids = sd["test_accounts"]["account_id"].values
        X_te = all_feats.loc[test_ids].values.astype(np.float32)
        log.info(f"  Test features: {X_te.shape}")

        # Load label features caches for test set
        log.info("  Checking for auxiliary label features caches...")
        ip_cache_file = CACHE / "ip_pairs_cache.parquet"
        community_cache_file = CACHE / "community_map_cache.parquet"
        ...
        if ip_cache_file.exists():
            ...
            from collections import defaultdict
            ip_acct_map = defaultdict(set)
            pf = pq.ParquetFile(ip_cache_file)
            for batch in pf.iter_batches(batch_size=2_000_000, columns=["account_id", "ip_address"]):
                accts = batch.column("account_id").to_pylist()
                ips = batch.column("ip_address").to_pylist()
                for a, ip in zip(accts, ips):
        ...
        community_map = pd.read_parquet(community_cache_file) if community_cache_file.exists() else None
        branch_acct_map = sd["accounts"][["account_id", "branch_code"]].copy()

        # Compute full-train label features (using all known mules)
        cleaned = clean_labels(sd, all_feats)
        tl = cleaned.set_index("account_id")
        train_ids = sorted(set(tl.index) & set(all_feats.index))
        y_train = tl.loc[train_ids, "is_mule"].values
        all_account_ids = list(all_feats.index)
        all_mule_ids = set(np.array(train_ids)[y_train == 1])
        ...
        N_FOLDS = 5  # must match training

        # ─── Base Model Predictions ───
        log.info("  Running base model predictions (7 models × 5 folds)...")
        lgb_test = np.mean([m.predict(X_te_aug_full) for m in lgb_fold_models], axis=0)
        xgb_test = np.mean([m.predict_proba(X_te_aug_sel)[:, 1] for m in xgb_fold_models], axis=0)
        cb_test = np.mean([m.predict_proba(X_te_aug_sel)[:, 1] for m in cb_fold_models], axis=0)
        et_test = np.mean([m.predict_proba(X_te_aug_sel)[:, 1] for m in et_fold_models], axis=0)
        hgb_test = np.mean([m.predict_proba(X_te_aug_sel)[:, 1] for m in hgb_fold_models], axis=0)

        # LF models use base features only (no label features, no selection)
        lf_test_pred = np.mean([m.predict_proba(X_te)[:, 1] for m in lf_fold_models], axis=0) if lf_fold_models else np.zeros(len(X_te))

        # Network models use network feature subset (from selected feature space)
        if net_fold_models and len(net_col_idx) > 0:
            net_test_pred = np.mean([m.predict_proba(X_te_aug_sel[:, net_col_idx])[:, 1] for m in net_fold_models], axis=0)
        else:
            net_test_pred = np.zeros(len(X_te))

        log.info(f"  Base predictions: LGB={lgb_test.mean():.4f}, XGB={xgb_test.mean():.4f}, "
                 f"CB={cb_test.mean():.4f}, ET={et_test.mean():.4f}, HGB={hgb_test.mean():.4f}, "
                 f"LF={lf_test_pred.mean():.4f}, NET={net_test_pred.mean():.4f}")

        # ─── Meta-Stacking (same pipeline as training) ───
        log.info("  Running meta-stacking...")

        # Track A: Core (5 models)
        core_tests = np.column_stack([lgb_test, xgb_test, cb_test, et_test, hgb_test])
        test_core_spread = core_tests.max(axis=1) - core_tests.min(axis=1)
        test_core_std = core_tests.std(axis=1)
        test_core_stack = np.column_stack([lgb_test, xgb_test, cb_test, et_test, hgb_test,
                                            test_core_spread, test_core_std])
        core_test = np.mean([m.predict(test_core_stack) for m in core_meta_models], axis=0)

        # Track B: Full (7 models + meta-features)
        all_model_tests = np.column_stack([lgb_test, xgb_test, cb_test, et_test, hgb_test, lf_test_pred, net_test_pred])
        test_pred_spread = all_model_tests.max(axis=1) - all_model_tests.min(axis=1)
        test_prediction_std = all_model_tests.std(axis=1)
        test_label_aug_mean = core_tests.mean(axis=1)
        ...
        full_test = np.mean([m.predict(test_full_stack) for m in full_meta_models], axis=0)

        # Track C: V2-style stacking (7 raw model predictions only — matches training)
        test_v2_stack = np.column_stack([
            lgb_test, xgb_test, cb_test, et_test, hgb_test, lf_test_pred, net_test_pred,
        ])
        v2_test = np.mean([m.predict(test_v2_stack) for m in v2_meta_models], axis=0)

        # Blender (meta-meta-learner) — 9 features matching training
        core_avg_test = core_tests.mean(axis=1)
        blend_features_test = np.column_stack([
            core_test, full_test, v2_test,
            core_test - full_test,
        ...
        blend_test = np.mean([m.predict(blend_features_test) for m in blend_meta_models], axis=0)

        # Simple blend
        simple_weights = np.array([0.25, 0.40, 0.35])
        simple_blend_test = (simple_weights[0] * core_test +
                             simple_weights[1] * full_test +
                             simple_weights[2] * v2_test)

        # Select best ensemble (same as training)
        ensemble_map = {
            "blend_learned": blend_test,
            "blend_simple": simple_blend_test,
            "core_stack": core_test,
            "full_stack": full_test,
            "v2_stack": v2_test,
        }
        final_test = ensemble_map[best_ensemble_name]
        log.info(f"  Using {best_ensemble_name} ensemble")

        # ─── Calibration ───
        if cal_method == "platt" and platt is not None:
            calibrated_test = platt.predict_proba(final_test.reshape(-1, 1))[:, 1]
        else:
            calibrated_test = iso.predict(final_test)
        log.info(f"  Calibrated test mean: {calibrated_test.mean():.6f}")
        ...
        log.info("  Generating submission CSV...")
        test = sd["test_accounts"]
        pred = pd.DataFrame({"account_id": test_ids, "is_mule": calibrated_test})
        sub = test.merge(pred, on="account_id", how="left")
        sub["is_mule"] = sub["is_mule"].fillna(0).clip(0, 1).round(4)

        # Temporal windows (estimate from full features)
        windows = estimate_windows({"test_ids": test_ids, "test_preds": calibrated_test,
        ...
        sub.loc[sub["is_mule"] < optimal_threshold, "suspicious_start"] = ""
        sub.loc[sub["is_mule"] < optimal_threshold, "suspicious_end"] = ""
        sub["suspicious_start"] = sub["suspicious_start"].fillna("")
        sub["suspicious_end"] = sub["suspicious_end"].fillna("")

        sub = sub[["account_id", "is_mule", "suspicious_start", "suspicious_end"]]
        sub.to_csv(BASE / "submission.csv", index=False)
        log.info(f"  Submission saved: {sub.shape}")
        log.info(f"  Mule > {optimal_threshold:.2f}: {(sub['is_mule'] > optimal_threshold).sum()}")
        log.info(f"  Mule > 0.5: {(sub['is_mule'] > 0.5).sum()}")
        log.info(f"  Mean: {sub['is_mule'].mean():.4f}")

        log.info("=" * 60)
        log.info("PREDICT MODE COMPLETE")
        log.info("=" * 60)

    else:
        # No saved models found — run full training pipeline
        main()
