...
def train_models(features, cleaned_labels, sd, edges_df, ip_acct_map, branch_acct_map, community_map, city_acct_map=None, state_acct_map=None):
    ...
    # ─── Blend: Meta-meta-learner on both tracks ───
    log.info("  Blending Track A + Track B via meta-meta-learner...")
    # Features: both track outputs + their difference + core model avg
    core_avg_oof = core_oofs.mean(axis=1)
    core_avg_test = core_tests.mean(axis=1)
    blend_features_oof = np.column_stack([
        core_oof, full_oof,                    # both tracks
        core_oof - full_oof,                   # disagreement between tracks
        core_avg_oof,                          # raw core average (v10-era signal)
        lf_oof,                                # LF raw (for direct comparison)
    ])
    ...
    blend_params = {
        "objective": "binary", "metric": "auc", "verbosity": -1,
        "n_estimators": 50, "num_leaves": 4, "max_depth": 2,
        "learning_rate": 0.05, "subsample": 0.8, "colsample_bytree": 0.8,
    ...
    all_aucs = {"lgb": lgb_auc, "xgb": xgb_auc, "cb": cb_auc,
                ...
                "core_stack": core_auc, "full_stack": full_auc,
    ...
    log.info(f"  Using {best_name} (AUC: {all_aucs[best_name]:.5f}) — dual-track ensemble")
